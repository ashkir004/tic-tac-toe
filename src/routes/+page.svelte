...
<main>
    <header>
        ...
        <div class="controls__container {screen.value === 'menu' ? 'hide' : ''}">
            <button class="btn btn-turn text-preset-4">
                ...
                {/if}
                Turn</button>
            <button class="btn btn-reset" aria-label="reset" onclick={() => reset = true}>
    ...
    <GameBoard updateScore={updateScore} resetScore={resetScore} turn={turn} setTurn={setTurn} reset={reset} setReset={setReset} />
