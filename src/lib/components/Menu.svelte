<script>
	...
	let { setPlayer1Mark, player1, setCPU, setTurn } = $props();
</script>

<section class="new-game {screen.value === 'menu' ? '' : 'hide'}">
		...
		<div class="mark-selection">
			...
			<div class="new-game__options">
				<button
					...
					class="new-game__option new-game__option-x {player1.mark === 'X' ? 'active' : ''}"
					aria-label="option X"
				>
					<svg viewBox="0 0 64 64" class="option-x" xmlns="http://www.w3.org/2000/svg"
						><path
			...
			<p class="new-game__starter text-preset-5-bold">Remember: X goes first</p>
		</div>
	</div>
...
<style>
	...
	.new-game__option:hover .option-x,
	.new-game__option:hover .option-o {
		fill: var(--slate-300);
		background-color: var(--slate-800);
	}
	...
	.new-game__option.active .option-x,
	.new-game__option.active .option-o {
		fill: var(--slate-900);
		background-color: var(--slate-300);
	}
