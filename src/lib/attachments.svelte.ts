...
export function trapFocusBoard(node: HTMLElement) {
	function updateFocus() {
		...
		if (overlay) {
			...
			focusableElements = node.querySelectorAll<HTMLElement>('.game-board button:not([disabled])');
		}
			...
			if (previousFocus) {
				try {
					...
					if (el && typeof el.focus === 'function' && document.contains(el)) {
						...
						const fallback =
							node.querySelector<HTMLElement>('.game-board button:not([disabled])') ||
							document.querySelector<HTMLElement>('.btn-reset');
